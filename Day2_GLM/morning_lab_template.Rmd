...
# Problem 3 · Line-transect detection of spider monkeys

> Spider monkey subgroups on a line transect. The detection function $g(x)$ is
> the probability a subgroup at perpendicular distance $x$ is seen; it is a curve,
> not a probability distribution. Each subgroup has a detection range
> $X \sim \mathrm{Exponential}(\lambda)$ with $\lambda = 0.02$/m, and
> $g(x) = P(X \ge x)$. (a) Define the survival function,
> write $S(x)$ for the Exponential, check $S(0) = 1$, and give $E[X]$.
> (b) Probability of *missing* a subgroup at 50 m. (c) Half-distance
