...
> Exponential detection function with rate $\lambda = 0.02$/m, where the
> latent detection range is $X \sim \mathrm{Exponential}(\lambda)$ and an
> animal at distance $x$ is seen when $X \ge x$. Write the survival function
> $S(x) = e^{-\lambda x}$ and its mean; compute the probability of *missing*
> an animal at 50 m (remember $S(50)$ is the probability it is *seen*); find
> the half-distance $\ln 2/\lambda$; explain what memorylessness implies
> about the detection hazard with distance.
