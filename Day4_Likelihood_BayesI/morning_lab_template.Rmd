...
> Load `data/whale.csv` (sex, blows). Fit the mixture $f(y) = \pi\,\text{Pois}(y;\lambda_1) + (1-\pi)\text{Pois}(y;\lambda_2)$ with `mle()` from at least 8 starts; discuss local optima and label switching; then validate $\hat\pi$, $\hat\lambda_1$ and $\hat\lambda_2$ against the `sex` column the mixture never used (1 = male, the lower-rate group).
