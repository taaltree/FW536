...
> Load `data/vonbert.csv` (sex, age, length). Run a grid search for $\kappa$, fit the full 3-parameter model with `mle()`, and test whether sex-specific residual variances are supported (LRT and AIC), starting from the lab's sex-specific growth-curve model.

```{r}
# Load exactly as in the lab:
# TheData <- read.csv(here::here("Day4_Likelihood_BayesI", "data", "vonbert.csv"))
...
> Load `data/whale.csv` (sex, blows). Fit the mixture $f(y) = \pi\,\text{Pois}(y;\lambda_1) + (1-\pi)\text{Pois}(y;\lambda_2)$ with `mle()`; explain why the mixture has no closed-form solution and must be fit numerically; refit from the 8 given starting values and describe how the answer depends on the start (label switching and stalled fits); explain label switching and enforce $\lambda_1 < \lambda_2$ by reparameterizing; then validate $\hat\pi$, $\hat\lambda_1$ and $\hat\lambda_2$ against the `sex` column the mixture never used (1 = male, the lower-rate group).

```{r}
# Load exactly as in the lab:
# TheData <- read.csv(here::here("Day4_Likelihood_BayesI", "data", "whale.csv"))
# blows <- TheData$blows
```
...
#
# For part (b), keep the control argument from the problem set when you call mle():
#   control = list(parscale = c(0.2, 1000, 0.03))
# r, K and sigma differ in scale by ~10^4, and optim() uses one step size for all
# three. Without parscale, K comes back stuck at its starting value (1000) with no
# warning. If your K_hat equals your start, that is the bug.
```
...
- [ ] Used **maximum likelihood only**, with no priors, posteriors, or MCMC.
