...
#   data/Vonbert.dat   (Problem 2, von Bertalanffy growth)
#   data/Whale.dat     (Problem 3, two-component Poisson mixture)
...
# Problem 2 — Von Bertalanffy growth from length-at-age data (Vonbert.dat)

> Load `data/Vonbert.dat` (sex, age, length). Run a grid search for $\kappa$, fit the full 3-parameter model with `mle()`, and test for a sex effect with a likelihood-ratio test.

```{r}
# Load exactly as in the lab:
# TheData <- matrix(scan("data/Vonbert.dat", skip = 1), byrow = TRUE, ncol = 3)
# Sex <- TheData[, 1]; Age <- TheData[, 2]; Len <- TheData[, 3]
```
...
# Problem 3 — Two-component Poisson mixture for whale blow counts (Whale.dat)

> Load `data/Whale.dat` (MF, Blows). Fit the mixture $f(y) = \pi\,\text{Pois}(y;\lambda_1) + (1-\pi)\text{Pois}(y;\lambda_2)$ with `mle()` from several starts; discuss multimodality and label switching.

```{r}
# Load exactly as in the lab:
# TheData <- matrix(scan("data/Whale.dat", skip = 1), byrow = TRUE, ncol = 2)
# blows <- TheData[, 2]
```
...
- [ ] Loaded `Vonbert.dat`, `Whale.dat`, and `Logistic.csv` from the `data/` folder.
