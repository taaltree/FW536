...
# Two problems load real course data from the data/ folder:
#   data/Logistic.csv  (Problem 7  -- Bayesian logistic r/K growth, derived MSY)
#   data/whale.csv     (Problem 10 -- the same blow counts as morning Problem 3)
# The Nimble scaffold for Problem 7 is Logistic_BayesianGrowth_Nimble.R.
```
...
> Refit the per-capita logistic model from morning Problem 4 in Nimble with priors $K \sim U(0, 4000)$, $r \sim U(0, 2)$, $\sigma \sim U(0, 2)$. (a, b) Report posteriors for $r$, $K$, $\sigma$ and check $r$ against the MLE. (c) Add derived nodes MSY $= rK/4$ and $N_{\max} = K/2$ and report medians and HPDIs. (d) Compute $\Pr(r > 0.22 \mid y)$ and compare the BCI and HPDI for $\sigma$. (e) Compute $\Pr(\text{MSY} < Q \mid y)$ for $Q = 55, 60, 62, 65, 70$ and defend a quota. (f) Refit under $K \sim U(0, 2000)$, $U(0, 4000)$ and $U(0, 20000)$, on the full data and on the first eight rows, and say when a bounded uniform prior deserves to be called uninformative. Scaffold: `Logistic_BayesianGrowth_Nimble.R`.
...
```{r}
# Problem 10 reuses the whale blow counts from MORNING Problem 3. That was a
# different .Rmd, so nothing carries over -- reload the data here. Both Nimble
# skeletons in problem_set.html refer to a vector called `blows`, so build it
# with exactly that name before you run either one.
TheData <- read.csv(here::here("Day4_Likelihood_BayesI", "data", "whale.csv"))   # columns: sex (1/2), blows
blows   <- TheData$blows                # the mixture treats sex as unobserved
length(blows)
```
...
- [ ] Loaded `Logistic.csv` (Problem 7) and `whale.csv` (Problem 10) from the `data/` folder.
