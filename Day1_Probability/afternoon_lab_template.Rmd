...
**Data:** Problem 9 uses `data/coyote_scat_counts.csv` from this day's folder. Open this
...
> (d) P(exactly 5 in 36 h), scaling lambda with the window.
...
```{r}
coyote <- read.csv(here::here("Day1_Probability", "data", "coyote_scat_counts.csv"))
str(coyote)
...
> (c) Derive the exact pooled mean and variance step by step, using only the definitions of
> mean and variance, no new theorem, and compare with (b).
...
- [ ] Loaded `data/coyote_scat_counts.csv` for Problem 9 (with the course project open).
